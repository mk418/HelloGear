...
- **Set options** — the gear button on a row opens rename, the helm and cloak toggles, and *Choose icon…*, which offers the set's own gear first, then the client's full icon list, and a search box. Search covers every icon it can put a name to — your gear and your spells; Classic Era reports the rest as bare file IDs with no name attached.
