...
- **The panel wears the addon's own chrome, not the character frame's.** Matching the character frame's border exactly was attempted at length and abandoned. It's the classic corner-piece frame — four texture quadrants on `PaperDollFrame`, no reusable template — and every route into it brought something worse: drawing the quadrants whole carried the paperdoll's slot recesses and the hardware its tabs bolt onto into the middle of the panel; slicing clean strips out sampled the wrong half of the texture file, because those regions already carry texcoords of their own that `SetTexCoord` discards; drawing the edge by hand from measured colours lost the bevel.

  What's here is the shared `ns.ApplyChrome` — the same border the quick menu and the flyouts use — filled with the character frame's header-band colour. It reads as a sibling of the character frame rather than a continuation of it, and unlike the alternatives it doesn't degrade. That is the whole justification: it was the version that looked best.

  One consequence to keep in mind: this border draws ~11px inside the frame's bounds (`ns.CHROME_INSET`), so the panel is inflated by that on every side and `PANEL_WIDTH` is the width you actually see, not the frame's. Getting this wrong looks exactly like a positioning bug.
