...
- **The character frame is modern chrome now.** 1.15.9 didn't just move Era's code onto the shared UI, it moved the *look* too: the character frame is the dark nine-slice frame, not the old tan parchment. `UI-DialogBox-Border` was never going to match it at any edge size — it's the wrong frame entirely. The panels use the same `DialogBorderDarkTemplate` nine-slice the rest of that UI uses, probed for once at load, with the old backdrop kept only as a fallback for a client that lacks it. `ns.ApplyChrome` is the single place this is decided, so no panel can drift.
