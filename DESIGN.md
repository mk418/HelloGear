...
- **The panel's edge is drawn, not borrowed.** Several rounds went into reusing the character frame's own artwork — it's the classic corner-piece frame, four texture quadrants on `PaperDollFrame`, with no reusable template behind it. Drawing the quadrants whole brought the paperdoll's slot recesses and the tab hardware along with them; slicing clean strips out with `SetTexCoord` kept sampling the wrong part of the texture, because those regions already carry texcoords of their own (the right-hand quadrants are 128 wide drawn from the left half of a 256-wide file) and `SetTexCoord` discards that mapping.

  Measuring what the frames actually look like settled it. The character frame and the social window edge themselves identically: a bright highlight on the outside (RGB ~141), a dark line immediately inside it (~20), then a mid tone that turns out to be the same colour as the header band. That's three flat colours, so the panel draws them. The reproduction is exact, there is no texture to sample wrongly, and nothing left to drift.

  The fill is that header band's colour. Note the input is lower than the measurement: the client renders a colour texture lighter than its nominal value, so the values are corrected to land on the header's rendered tone.
