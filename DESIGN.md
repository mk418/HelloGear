...
- **The panel wears the character frame's own artwork.** The character frame is the classic corner-piece frame: four texture quadrants laid out 384x512 on `PaperDollFrame`, with no reusable template behind it. Several look-alike borders were tried against it — the tan `UI-DialogBox-Border`, then the dark `DialogBorderDarkTemplate` — and each was visibly not it. So the panel reads those four regions off `PaperDollFrame` at runtime and draws them again, shifted right so their right border lands on the panel's right edge. Everything to the left runs off behind the character frame, which the panel deliberately draws *underneath*. The result isn't a border matched to the character frame's; it is the character frame's, carrying on. `/hg dock art` is what dumped the layout.
