...
- **The panel wears the character frame's own artwork.** The character frame is the classic corner-piece frame: four texture quadrants laid out 384x512 on `PaperDollFrame`, with no reusable template behind it. Two look-alike borders were tried against it — the tan `UI-DialogBox-Border`, then the dark `DialogBorderDarkTemplate` — and both were visibly not it.

  Drawing the quadrants wholesale doesn't work either: they carry the paperdoll's slot recesses, and the bottom border carries the hardware the character tabs bolt onto, which lands orphaned in the middle of a panel that has no tabs. So each edge is a *clean strip* sliced out of the same textures with `SetTexCoord` — plain runs of border with nothing attached — and the bottom reuses the top border flipped, the top being the one horizontal edge with no hardware on it. The bottom-right corner mirrors the top-right for the same reason.

  Every piece anchors to the panel's own edges, so unlike the wholesale approach this doesn't depend on the panel and the artwork agreeing about where the frame's corner is. `/hg dock art` dumps the layout; `/hg dock border <n>` re-slices at a different border thickness for calibrating without a reload.
